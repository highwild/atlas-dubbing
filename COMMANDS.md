...
| `YTDUB_CHARS_PER_SECOND_BY_LANGUAGE` | `{"hi": 12}` | per-language rates, as JSON. Measured on tangi.wav: the synthesizer speaks 6.6 Hindi characters per second where Spanish manages 8.2 and Dutch 9.4, so the shared 15 asked Hindi for roughly twice the text its slot holds and the fit compressed to its 3x cap — a fast-forward, not speech. |
| `--min-confidence P` | `0.45` | drop transcribed segments below this mean word probability as probable noise |
| `YTDUB_TTS_ATTEMPTS` | `4` | seeds to try before a clip is given up on. Chatterbox's loop is seed-dependent, not text-dependent: one Hindi line looped on four consecutive seeds and came back clean on four of five seeds thousands apart, so retries stride the seed rather than adding 1. |
| `YTDUB_TTS_EXPECTED_CHARS_PER_SECOND` | `{"hi": 8}` | what a language is expected to produce, used only to tell slow speech from a loop. Too high and ordinary clips are dropped as runaway: at the Latin default of 12, six real Hindi lines were thrown away. |
| `YTDUB_TTS_STUCK_GAP` | `3` | how far a one-word line may be moved to be spoken with a line of its own speaker. Beyond it the line is left unspoken and reported: a lone word is what crashes or loops the synthesizer. Merge it with a neighbour in the review SRT to get it into the dub. |
